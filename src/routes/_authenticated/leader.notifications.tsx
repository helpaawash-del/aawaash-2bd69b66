...
function NotificationsContent() {
  ...
  const markAll = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ["leader", "notifications"] }),
  });
    ...
    <DashboardShell role={role || "team_leader"} profile={profile}>
      <header className="flex flex-wrap items-end justify-between gap-3">
        ...
        <button
          ...
          className="inline-flex h-10 items-center gap-1.5 rounded-2xl border border-border bg-surface px-3 text-xs font-semibold text-foreground shadow-[var(--shadow-soft)] transition-all hover:-translate-y-0.5 disabled:opacity-50"
        >
