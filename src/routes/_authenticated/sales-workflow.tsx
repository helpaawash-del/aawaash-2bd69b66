...
function SalesWorkflowInner() {
  ...
  const role = overviewQ.data?.role ?? "member";

  return (
    <DashboardShell
      eyebrow="Sales"
      title="Sales Workflow"
      description="Every property sale — from draft to registration — flows through this workspace."
      actions={
        <Link
          to="/sales-workflow/new"
          className="btn-primary btn-primary-hover inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold text-primary-foreground"
        >
    ...
    >
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <StatCard
          ...
          label="Total Sales"
          value={overviewQ.data?.totalSales ?? 0}
          accent="primary"
        />
        <StatCard
          icon={<Clock className="size-5" />}
          label="Awaiting Approval"
          value={overviewQ.data?.pendingCount ?? 0}
          accent="gold"
        />
        <StatCard
          icon={<ClipboardCheck className="size-5" />}
          label="Approved"
          value={overviewQ.data?.approvedCount ?? 0}
          accent="leaf"
        />
        <StatCard
      ...
      <SectionCard
        title="Sales"
        description="Filter, search, and open any sale to see its complete lifecycle."
      >
        ...
        {listQ.isLoading ? (
          ...
          <div className="mt-4">
            <EmptyState
              ...
              title="No sales yet"
              body="Draft a new sale to get started."
            />
          </div>
        ) : (
