...
import { getTeamLeaderboard } from "@/lib/leader.functions";

export const Route = createFileRoute("/_authenticated/leader/leaderboard")({
  component: LeaderboardPage,
  head: () => ({ meta: [{ title: "Leaderboard — Aawash" }] }),
});
...
function LeaderboardContent() {
  ...
  const fn = useServerFn(getTeamLeaderboard);
  const { data, isLoading } = useQuery({ queryKey: ["leader", "board"], queryFn: () => fn() });
  ...
  return (
    <DashboardShell role={role || "team_leader"} profile={profile}>
      <header>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Updates automatically after every sale.</p>
      </header>
      ...
      {isLoading ? (
        ...
        <section className="mt-6">
          <EmptyState
            ...
            body="Rankings will appear here once your members start closing sales."
          />
        </section>
      ) : (
        <>
          ...
          <section className="mt-6 grid grid-cols-3 gap-3">
            {[1, 0, 2].map((i) => {
              ...
              return (
                <div
                  key={m.id}
                  className={`glass-card relative flex flex-col items-center rounded-3xl p-3 text-center shadow-[var(--shadow-soft)] ${orderCls} ${heights[i]}`}
                >
                  <Trophy
                    size={20}
                    className={
                      tier === 1 ? "text-gold" : tier === 2 ? "text-primary" : "text-muted-foreground"
                    }
                  />
                  ...
                  <div className="mt-2 truncate text-xs font-bold text-foreground">
                    {m.full_name.split(" ")[0]}
                  </div>
