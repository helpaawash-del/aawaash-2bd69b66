 ...
 * Tilts along with mouse/touch position for a hologram-desk feel and
 * gracefully collapses to a static render when the user prefers reduced
 * motion. Purely presentational — never blocks pointer events on siblings.
 */
export function Parallax3DBuilding({
  ...
  const onMove = useCallback(
    (clientX: number, clientY: number) => {
      ...
      setTilt({ x: -8 + ny * -14, y: 14 + nx * 22 });
    },
    [reduced],
  );

  const reset = useCallback(() => setTilt({ x: -8, y: 14 }), []);
