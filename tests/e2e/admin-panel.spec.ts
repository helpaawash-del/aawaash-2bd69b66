...
test.describe("Admin master dashboard", () => {
  ...
  test("create Team Leader → detail page opens on Members tab", async ({ page }) => {
    await adminLogin(page);
    await enterPasscode(page, PASSCODE);
    await page.waitForURL(/\/admin\/?$/);

    const mobile = `98${Date.now().toString().slice(-8)}`;
    const slot = page.locator("form", { hasText: "Team Leader A" }).first();
    if (await slot.count()) {
      ...
      await slot.getByRole("button", { name: /save leader/i }).click();
      await expect(page.getByText(new RegExp(`Login ID: ${mobile}`))).toBeVisible({ timeout: 15_000 });

      await page.getByRole("link", { name: /open leader page/i }).first().click();
