...
AS $function$
BEGIN
  -- Trusted paths: no JWT (service role / jobs), admins, or updates running
  -- inside a SECURITY DEFINER routine owned by a privileged role (the wallet /
  -- commission engine). Direct client updates run as the `authenticated` role.
  IF auth.uid() IS NULL
